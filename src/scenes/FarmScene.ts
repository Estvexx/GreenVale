...
export class FarmScene extends Phaser.Scene {
    ...
    preload() {
        for (const skin of SKINS) {
            this.load.image(`${skin}_idle`, `assets/images/players/player_walk2_${skin}.png`);
            this.load.image(`${skin}_walk`, `assets/images/players/player_walk1_${skin}.png`);
        }

        this.load.tilemapTiledJSON("farm", "assets/maps/mapa_final.tmj");
        this.load.image("Armazem_Green",  "assets/images/map_images/Supplies.png");
        ...
        this.load.image("Fields TileSet", "assets/images/map_images/FieldsTileset.png");
        this.load.image("Ground",         "assets/images/map_images/TileSet_V1.png");
        this.load.image("Ground Details", "assets/images/map_images/TX Props.png");
        this.load.image("Tenda",          "assets/images/map_images/1.png");
        this.load.image("Tenda Venda",    "assets/images/map_images/4.png");
        this.load.image("Terra Lavrada",  "assets/images/map_images/terrain.png");
    ...
    create() {
        ...
        const tsGroundDetails = map.addTilesetImage("Ground Details", "Ground Details")!;
        const tsTerraLavrada  = map.addTilesetImage("Terra Lavrada",  "Terra Lavrada")!;
        const tsCercas        = map.addTilesetImage("Cercas",         "Cercas")!;
        const tsTenda         = map.addTilesetImage("Tenda",          "Tenda")!;
        const tsArmazem       = map.addTilesetImage("Armazem_Green",  "Armazem_Green")!;
        const tsTileSet       = map.addTilesetImage("TileSet",        "TileSet")!;
        const tsFields        = map.addTilesetImage("Fields TileSet", "Fields TileSet")!;
        ...
        map.createLayer("Ground Details", [tsGroundDetails, tsCercas, tsTileSet])?.setDepth(1);
        map.createLayer("Farmable Layer", tsTerraLavrada)                        ?.setDepth(2);
        map.createLayer("Decoration",     [tsTenda, tsTendaVenda, tsArmazem])    ?.setDepth(3);

        const trunkLayer   = map.createLayer("Trees",       tsTrees) as Phaser.Tilemaps.TilemapLayer;
        const canopyLayer  = map.createLayer("AbovePlayer", tsTrees) as Phaser.Tilemaps.TilemapLayer;
